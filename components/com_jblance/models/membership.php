 ...
 class JblanceModelMembership extends JModelLegacy {
	...
	function getEscrow(){
		...
		if(empty($projects)){
			...
			$lists 	= JHTML::_('select.genericlist', $types, 'project_id', "class='span4 required' size='5' onclick='fillProjectInfo();'", 'value', 'text', '');
		}
	...
	function getManagepay(){
		...
		$query = "SELECT * FROM #__jblance_withdraw WHERE user_id= ".$user->id." ORDER BY approved, date_withdraw DESC";
		$db->setQuery($query);
		$db->execute();
		$total = $db->getNumRows();
		...
		$query = "SELECT * FROM #__jblance_deposit WHERE user_id= ".$user->id." ORDER BY approved, date_deposit DESC";
		$db->setQuery($query);
		$db->execute();
		$total = $db->getNumRows();
	...
	function buildPlanInfo($planId){
		...
		if($ugInfo->allowBidProjects){
			...
			//$infos[$i]->value = "<span class=hasTip title=\'".JText::_('COM_JBLANCE_WHICHEVER_HIGHER')."\'>".JblanceHelper::formatCurrency($planInfo->flFeeAmtPerProject).' '.JText::_('COM_JBLANCE_OR').' '.$planInfo->flFeePercentPerProject.'%'."</span>";
		...
		if($ugInfo->allowPostProjects){
			$infos[$i] = new stdClass();
			$infos[$i]->key = JText::_('COM_JBLANCE_PROJECT_COMMISSION');
			//$infos[$i]->value = "<span class=hasTip title='".JText::_('COM_JBLANCE_WHICHEVER_HIGHER')."'>".JblanceHelper::formatCurrency($planInfo->buyFeeAmtPerProject).' '.JText::_('COM_JBLANCE_OR').' '.$planInfo->buyFeeAmtPerProject.'%'."</span>";
